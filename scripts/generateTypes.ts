...
const getUniqueDeploymentKey = (
  chainId: string,
  baseName: string
): string => {
  if (!deployments[chainId] || !deployments[chainId][baseName]) {
    return baseName;
  }

  let suffix = 2;
  let candidate = `${baseName}_${suffix}`;
  while (deployments[chainId][candidate]) {
    suffix += 1;
      ...
      if (tx.transactionType === 'CREATE' && tx.contractName && tx.contractAddress) {
        ...
        if (contractName === 'ERC1967Proxy') {
          if (lastContractName) {
            ...
            const deploymentKey = getUniqueDeploymentKey(chainId, proxyName);
            deployments[chainId][deploymentKey] = {
          ...
          const deploymentKey = getUniqueDeploymentKey(chainId, contractName);
          deployments[chainId][deploymentKey] = {
            address: contractAddress,
...
for (const chainId in deployments) {
  for (const deploymentKey in deployments[chainId]) {
    const name = deployments[chainId][deploymentKey].contractName.replace('Proxy', '');
